...
if __name__ == "__main__":
    ...
    if args.mode == "batch":
        # Get dataset
        dataset = DataSet(args.source, preprocess_config)
        print(dataset)
        batchs = DataLoader(
            dataset,
            batch_size=8,
    ...
    if args.mode == "single":
        ...
        batchs = [(ids, raw_texts, speakers, texts, text_lens, max(text_lens), emotions)]
        # batchs = []
        # for s in range(153):
        #     batchs.append((ids, raw_texts, np.array([s]), texts, text_lens, max(text_lens), emotions))
