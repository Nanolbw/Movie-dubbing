...
from enhance_speach import speech_enhance
...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    mel_len_pred = predictions[9][0].item()
    mel_len = targets[7][0].item()
    mel_target = targets[6][0, :mel_len].detach().transpose(0, 1)
    mel_prediction = predictions[1][0, :mel_len_pred].detach().transpose(0, 1)
    ...
    with open(
        ...
        # stats.json 在preprocess中生成 记录了 min max mean std等信息
        stats = json.load(f)
    ...
    if vocoder is not None:
        ...
        wav_reconstruction = speech_enhance(wav_reconstruction, 22050)
        wav_prediction = speech_enhance(wav_prediction, 22050)
    else:
...
def synth_multi_samples(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    for i in range(len(predictions[0])):
        ...
        mel_len_pred = predictions[9][i].item()
        mel_len = targets[7][i].item()

        mel_target = targets[6][i, :mel_len].detach().transpose(0, 1)
        mel_prediction = predictions[1][i, :mel_len_pred].detach().transpose(0, 1)
        duration = targets[11][i, :src_len].detach().cpu().numpy()

        # todo:
        duration_prediction = predictions[4][i, :].detach().cpu().numpy()
        #cofs.append((duration.sum(), duration_prediction.sum()))
        # if preprocess_config["preprocessing"]["pitch"]["feature"] == "phoneme_level":
    ...
    # return fig, wav_reconstruction, wav_prediction, basename
    return wav_reconstructions, wav_predictions, basenames, speakers, emotions
...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path):
    ...
    speakers = targets[2]
    emotions = targets[6]
    # whether handle the gt as well
    # if preprocess_config["with_gt"]:
    #     mels_gt = targets[7]
    #     pitches_gt = targets[8]
    #     energies_gt = targets[9]
    #     durations_gt = targets[10]
    # 
    for i in range(len(predictions[0])):
        basename = basenames[i]
        # prediction
        ...
        # ground-truth
        if preprocess_config["with_gt"]:
            src_len_gt = targets[4][i].item()
            mel_len_gt = targets[11][i].item()
            mel_gt = targets[7][i, :mel_len_gt].detach().transpose(0, 1)
            duration_gt = targets[10][i, :src_len_gt].detach().cpu().numpy()

        if preprocess_config["preprocessing"]["pitch"]["feature"] == "phoneme_level":
            pitch = predictions[2][i, :src_len].detach().cpu().numpy()
            pitch = expand(pitch, duration)
            if preprocess_config["with_gt"]:
                pitch_gt = targets[8][i, :src_len_gt].detach().cpu().numpy()
                pitch_gt = expand(pitch_gt, duration_gt)
        else:
            pitch = predictions[2][i, :mel_len].detach().cpu().numpy()
            if preprocess_config["with_gt"]:
                pitch_gt = targets[8][i, :mel_len_gt].detach().cpu().numpy()

        if preprocess_config["preprocessing"]["energy"]["feature"] == "phoneme_level":
            energy = predictions[3][i, :src_len].detach().cpu().numpy()
            energy = expand(energy, duration)
            if preprocess_config["with_gt"]:
                energy_gt = targets[9][i, :src_len_gt].detach().cpu().numpy()
                energy_gt = expand(energy_gt, duration_gt)
        else:
            energy = predictions[3][i, :mel_len].detach().cpu().numpy()
            if preprocess_config["with_gt"]:
                energy_gt = targets[9][i, :mel_len_gt].detach().cpu().numpy()
        ...
        if preprocess_config["with_gt"]:
            fig_gt = plot_mel(
                [
                    (mel_gt.cpu().numpy(), pitch_gt, energy_gt),
                ],
                stats,
                ["Ground-Truth Spectrogram"],
            )
            plt.savefig(os.path.join(path, "{}_gt.png".format(basename)))
            plt.close()
    ...
    # ground-truth
    if preprocess_config["with_gt"]:
        mel_gts = targets[7].transpose(1, 2)
        lengths_gt = targets[11] * preprocess_config["preprocessing"]["stft"]["hop_length"]
        wav_gts = vocoder_infer(
            mel_gts, vocoder, model_config, preprocess_config, lengths=lengths_gt
        )

    sampling_rate = preprocess_config["preprocessing"]["audio"]["sampling_rate"]
    # predictions
    # for wav, basename in zip(wav_predictions, basenames):
    for wav, basename, speaker, emotion in zip(wav_predictions, basenames, speakers, emotions):
        wavfile.write(os.path.join(path, "{}_{}_{}.wav".format(basename, speaker, emotion)), sampling_rate, wav)
    # ground-truths
    if preprocess_config["with_gt"]:
        for wav, basename in zip(wav_gts, basenames):
            wavfile.write(os.path.join(path, "{}_gt.wav".format(basename)), sampling_rate, wav)
